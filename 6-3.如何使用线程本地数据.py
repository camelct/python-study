'''
    可参考 https://www.cnblogs.com/smulngy/p/9010189.html

    问题：如何使用线程本地数据?

    实际案例
        我们实现了一个web视频监控服务器，服务器端采集摄像头数据，客户端使用浏览器通过http请求接收数据.
        服务器使用推送的方式(multipart/x-mixed-replace)一直使用一个tcp连接向客户端传递数据.
...
# 程序结构  (单线程)
'''
Streamer     从摄像头采集数据（相当于数据源）
   ↓
Retriever    为了灵活的从数据源中获取数据，传输到不同的设备上，从而再次抽象出来
   ↓
Handler      负责http请求的， 从上面拿数据，发送到服务端
_________
  ↑  ↓ http
 client
'''
...
# threading.Thread(target=f).start() # 在子线程中不能访问

def f2():
    l.x = 5


threading.Thread(target=f2).start() # 在子线程中不能访问
